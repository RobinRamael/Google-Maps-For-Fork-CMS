<?php
/*
 * Project:     GoogleMapAPI V3 for Fork CMS: a PHP library inteface
 *              to the Google Map API v3 to use from fork-cms.
...
For database caching, we call the getDB function of Fork
CMS, so make sure that this model is reachable from where the code is
called.  You can change the caching table name by setting the
_db_cache_table variable. The class expects to find a table which is
constructed like the following (if the _db_cache_table variable is
not set, we default to this tablename):
...
class GoogleMapAPI {
  ...
  function setCached($cached) {
	...
	*/
    ...
    /**
     * Get the spoon databae from the correct model (backend/frontend).
     *
     */
    private function getDatabase($writeable) {
        if (class_exists('BackendModel'))
            return BackendModel::getDB($writeable);
        else if (class_exists('FrontendModel'))
            return FrontendModel::getDB($writeable);
        else
            die("Neither the BackendModel nor the FrontendModel were loaded.");
    }
    ...
    function getCache($address) {
        ...
        $_ret = $this->getDatabase(false)->getRecord("SELECT lat, lon FROM " . $this->_db_cache_table .
                                                       " WHERE address = ?
                                                         LIMIT 1;",
                                                      $address);

        return ($_ret === null ? false : $_ret);
    }
    ...
    function putCache($address, $lon, $lat) {
        ...
        if(!$this->cached || (strlen($address) == 0 || strlen($lon) == 0 || strlen($lat) == 0))
           return false;


        $this->getDatabase(true)->insert($this->_db_cache_table, array('address' => $address,
                                                                        'lon' => $lon,
                                                                        'lat' => $lat));
        return true;
    }
